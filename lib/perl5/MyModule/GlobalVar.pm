...
@EXPORT = qw($PIPELINE_PATH $BIN_PATH $DB_PATH $TOOL_PATH $CONFIG_FILE $DB_LIST_BED
             $REF_H_CONFIG $PYTHON3 $MAX_AVAILEBLE_MEM $REF_H_DBPATH $QUEUE_PM);
...
our $QUEUE_LIST     = "$DB_PATH/queue.list";
...
# Kraken db info
&getBEDbInfo($DB_LIST_BED, $QUEUE_LIST);
...
sub getBEDbInfo{
    ...
    while (<IN>) {
      ...
      my @tmp = split;
      $pm_queue{$tmp[0]} = $tmp[1];
    }
    close IN;

    our $QUEUE_PM = \%pm_queue;
